...
# Lazily-created cached Secrets Manager client.
# Use get_secrets_client() to obtain a client (injectable for tests).
_secrets_client = None

def _get_secrets_client(secrets_client=None):
    """Return the provided secrets_client, or create/return a cached boto3 client.

    If a `secrets_client` is supplied we return it without mutating the module
    cache, avoiding test pollution when tests inject fake clients.
    """
...
class CloudWatchLog(TypedDict, total=False):
    ...
    Both `stack_trace` and `service` are optional in practice; code
    defensively uses .get(...) with defaults when these keys are missing.
    """
...
def process_event(
    ...
    *,
    ...
    timestamp_in_seconds = _resolve_timestamp(decoded_message.get("logEvents", []), time_func)
    ...
    try:
        if len(decoded_message["logEvents"]) > 0:
            log_events: list[CloudWatchLog] = [json.loads(log_event["message"]) for log_event in decoded_message["logEvents"]]
        ...
        slack_message = create_slack_message(
            ...
            [
                f"{evt['message'][:100]}..."
                for evt in decoded_message["logEvents"][1:]
            ],
        )
